...
export function DeleteExamGroupModal() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(`Exam Group "${groupTitle || "Group"}" deleted successfully.`)
      closeModal()
    } catch (err: any) {
      ...
      >
        ...
        <DialogHeader className="pt-1">
          <DialogDescription className="font-body-md text-sm leading-relaxed text-on-surface-variant">
            ...
            <span className="font-bold text-on-surface">
              &quot;{groupTitle || "selected group"}&quot;
            </span>
            ? All associated item assignments and calculated student results will be permanently removed. Standalone exams will NOT be affected.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-700">
          ...
          <p className="leading-snug">
            This action cannot be undone. Any merit list generation calculations dependent on this group will be deleted.
          </p>
        </div>
