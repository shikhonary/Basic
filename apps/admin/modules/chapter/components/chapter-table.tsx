...
import { MoreVertical, Pen, Trash, GitBranch } from "lucide-react"
      ...
      {isLoading ? (
        <div className="p-4 md:p-12">
          ...
          <div className="grid grid-cols-1 gap-3 md:hidden">
            {Array.from({ length: 4 }).map((_, idx) => (
              <Skeleton
                key={idx}
                className="h-[148px] w-full rounded-xl bg-surface-container-high"
              />
            ))}
          </div>
        ...
        <div className="mx-auto max-w-md p-8 sm:p-16 text-center text-error">
          ...
          <p className="mt-2 font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            Failed to retrieve chapter records. Check network configurations or try refreshing the dashboard.
          </p>
        </div>
      ) : items.length === 0 ? (
        <div className="mx-auto max-w-md p-8 sm:p-16 text-center">
          ...
          <p className="mt-2 font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            No chapter records matched your search parameters. Establish your first subject chapter to get started.
          </p>
          ...
          <div className="grid grid-cols-1 gap-3 p-3.5 sm:p-4 md:hidden">
            {items.map((item) => (
              ...
              >
                ...
                {/* Parent Subject Mapping */}
                <div className="flex flex-col gap-1 border-t border-outline-variant/20 pt-2.5 text-xs">
                  <div className="flex items-center gap-1.5 text-on-surface-variant font-medium">
                    <span className="font-bold text-[9px] uppercase tracking-wider text-outline">
                      Subject:
                    </span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-outline-variant bg-surface-container-low px-4 sm:px-6 py-4">
            <div className="flex items-center gap-4 flex-wrap justify-center sm:justify-start">
              <p className="font-body-md text-xs sm:text-sm text-on-surface-variant">
                Showing <span className="font-bold text-on-surface">{displayStart}-{displayEnd}</span> of <span className="font-bold text-on-surface">{totalItems}</span> chapters
              </p>
              {onLimitChange && (
