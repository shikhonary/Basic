...
export function UserFilterBar({ filters, onFilterChange, onResetFilters }: UserFilterBarProps) {
  ...
  const hasActiveSearch = Boolean(filters.search && filters.search.trim() !== "")
  const hasActiveRole = Boolean(filters.role && filters.role !== "All")
  const hasActiveStatus = Boolean(filters.status && filters.status !== "All")
  const hasActiveSort = Boolean(filters.sort && filters.sort !== "desc")
  const hasAnyFilter = hasActiveSearch || hasActiveRole || hasActiveStatus || hasActiveSort
          ...
          <DrawerContent className="p-6 space-y-5 bg-white border-t border-outline-variant/40">
            <DrawerHeader className="p-0 text-left">
              ...
              <DrawerDescription className="text-xs text-on-surface-variant">
                Select user roles, verification statuses and sorting options.
              </DrawerDescription>
            </DrawerHeader>
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            ...
            {hasActiveSort && (
              <Badge
                ...
                className="inline-flex items-center gap-1 rounded-md border border-outline-variant/40 bg-surface-container-high px-2 py-1 text-[11px] sm:text-xs font-medium text-on-surface hover:bg-surface-container-highest cursor-default normal-case tracking-normal shrink-0"
              >
                <span>Sort: {getSortLabel(filters.sort ?? "desc")}</span>
                <button
                  type="button"
